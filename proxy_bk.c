...
int setGETheader(const char* buff, char* request, char* hostname){
	...
	char* uri;
	char temp_host[100];
    char temp_buffer[MAXBUFFERSIZE];
    char temp_get[MAXBUFFERSIZE];
    char fullhost[MAXBUFFERSIZE];
	int urilength;
	if(first == NULL){
...
//		urilength = strlen(hostend) - strlen(uriend);
        ...
        sprintf(temp_get,"GET %s HTTP/1.0\r\n",uri);
        free(uri);
	}
	...
	if( takehostname(buff,temp_host) ){
        sprintf(fullhost,"Host: %s\r\n",temp_host);
        sprintf(request,"%s%s",temp_get,fullhost);
		return 1;
	}
	else{
        sprintf(fullhost,"Host: %s\r\n",hostname);
